...
/// A entropy decoder. Called "range decoder" in the specification.
///
...
pub(crate) struct EntropyDecoder<'d> {
    /// Buffered input.
    buffer: &'d [u8],
...
impl<'d> Tell for EntropyDecoder<'d> {
    #[inline(always)]
...
impl<'d> EntropyDecoder<'d> {
    ...
    pub(crate) fn new(buffer: &'d [u8]) -> Self {
        ...
        decoder.remainder = decoder.next_byte();
    ...
    fn next_byte_from_end(&mut self) -> u8 {
        if self.end_offs < self.buffer.len() {
            self.offs += 1;
            self.buffer[self.buffer.len() - self.end_offs]
        } else {
            0
        }
    }
    ...
    pub(crate) fn uint(&mut self, mut ft: u32) -> Result<u32, DecoderError> {
        ...
        if ftb > UINT_BITS {
            ...
            Err(DecoderError::InternalError("entropy decoder: uint: t>ft"))
        } else {
            ft += 1;
            let s = self.decode(ft);
            self.update(s, s + 1, ft);
    ...
    fn bits(&mut self, bits: u32) -> u32 {
        ...
        if available < bits {
            loop {
                window |= u32::from(self.next_byte_from_end()) << available;
                available += SYM_BITS;
