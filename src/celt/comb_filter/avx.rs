...
pub(crate) fn comb_filter_const(
    ...
    unsafe {
        ...
        (0..n - 7).into_iter().step_by(8).for_each(|i| {
            let yi = _mm256_loadu_ps(x[x_offset + i..].as_ptr());
            ...
            let yi = _mm256_add_ps(yi, _mm256_mul_ps(g10v, x2v));
            let yi = _mm256_add_ps(yi, _mm256_mul_ps(g11v, _mm256_add_ps(x3v, x1v)));
            let yi = _mm256_add_ps(yi, _mm256_mul_ps(g12v, _mm256_add_ps(x4v, x0v)));
...
pub(crate) fn comb_filter_const_inplace(
    ...
    unsafe {
        ...
        let mut j = 0;
        (0..n - 7).into_iter().step_by(8).for_each(|i| {
            let yi = _mm256_loadu_ps(y[y_offset + i..].as_ptr());
